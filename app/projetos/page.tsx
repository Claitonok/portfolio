  ...
  return (
    <main className="bg-slate-950 text-white min-h-screen">
      ...
      <section className="px-6 py-20 max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold text-center md:text-4xl mb-16 bg-linear-to-r
        ...
        <div className="grid md:grid-cols-2 gap-10">
          {projects.map((project, index) => (
            <div
              key={index}
              className="group bg-slate-900/60 backdrop-blur-lg border border-slate-800 p-8 rounded-3xl 
              shadow-lg hover:shadow-blue-500/10 transition-all duration-300 hover:-translate-y-2"
            >
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {project.tech.map((tech, i) => (
                  <span
                    key={i}
                    className="text-xs px-3 py-1 bg-slate-800 
              ...
              <Link
                ...
                className="inline-block text-sm font-medium
                 text-blue-400 group-hover:text-cyan-400 transition"
              >
                Ver no GitHub →
              </Link>
            </div>
